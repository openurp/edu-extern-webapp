...
[@b.head/]
  [@b.grid items=externGrades var="externGrade"]
    [@b.gridbar]
      ...
      [#else]
        var bar1=bar.addMenu("导出", action.exportData("std.user.code:学号,std.user.name:姓名,school.name:校外学校,level.name:培养层次,category:教育类别,majorName:外校专业,courseName:外校课程,scoreText:外校得分,credits:外校学分,acquiredOn:获得日期,updatedAt:录入时间,courseGradeSize:已认定课数"));
        bar1.addItem("学分银行", action.exportData("info.std.user.name:姓名,info.std.person.code:身份证号,info.school.code:转换学校代码,original.course.code:原课程来源代码,info.majorName:原专业名称,info.courseName:原课程名称,info.school.name:原办学机构,info.level.code:原教育层次代码,info.category.code:原教育类别代码,info.credits:原学分,original.course.creditHours:原学时,info.scoreText:原成绩,info.acquiredOn:获得时间,info.std.level.code:转换后教育层次代码,courseGrade.course.code:转换后课程代码,courseGrade.course.name:转换后课程名称,courseGrade.course.credits:转换后学分", "xls", "fileName=学分银行成绩-外校成绩&dataInSource=courseGrade"), "excel.png");
      [/#if]
