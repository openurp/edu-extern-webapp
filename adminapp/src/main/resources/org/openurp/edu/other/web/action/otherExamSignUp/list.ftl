...
[@b.form name="otherExamSignUpListForm" action="!search" target="otherExamSignUpList"]
    [@b.grid items=otherExamSignUps var="otherExamSignUp"]
        ...
        [@b.gridbar]
                ...
                [#--
                bar.addItem("管理员主动对账","updateBillState()");
                [#if !payOpen]
                    [#list payStates as payState]
                        [#if payState_index==0]
                            var menu = bar.addMenu("置为${payState.name}","savePayState(${payState.id})");
                        [#else]
                            menu.addItem("置为${payState.name}","savePayState(${payState.id})");
                        [/#if]
                    [/#list]
                [/#if]
                function updateBillState(){
                    ...
                    if(ids ==null || ids ==""){
                        if(confirm("将对所有未支付订单进行对账,可能花费较长时间,是否继续?")){
                            return;
                        }
                    }else{
                        bg.form.addInput(form,"otherExamSignUp.ids",ids);
                    }
                    bg.form.submit(form,"${b.url('otherExamSignUp!updateBillState')}");
                }
                --]
                ...
                function downloadAvatorBatch(){
                    ...
                    bg.form.addInput(form,"otherExamSignUp.ids",ids);
                    bg.form.submit(form,"${b.url('otherExamSignUp!downloadAvatorBatch')}","_self");
                }
        [/@]
        [/#if]
        [@b.row]
            ...
            [@b.col property="std.name" title="std.name" width="14%"]
                [@b.a href="otherExamSignUp!info?otherExamSignUp.id="+otherExamSignUp.id title="${b.text('ui.otherExamSignUp.frame')}" ]${(otherExamSignUp.std.name)!}[/@]
            [/@]
            ...
            [@b.col property="feeOfMaterial" title="材料费" width="6%"]${(otherExamSignUp.feeOfMaterial)!"--"}[/@]
            [@b.col property="feeOfOutline" title="考纲费" width="6%"]${(otherExamSignUp.feeOfOutline)!"--"}[/@]
            ...
            [@b.col property="signUpAt" title="报名时间" width="9%"]${((otherExamSignUp.signUpAt)?string("yyyy-MM-dd"))!}[/@]
        [/@]
    [/@]
[/@]
<script>
    ...
    function savePayState(state){
        ...
        form.action="otherExamSignUp!savePayState.action?otherExamSignUp.payState="+state;
        bg.form.submit(form);
    }    
    ...
    function exportList(form){
        ...
        bg.form.submit(form, "otherExamSignUp!export.action","_self");
    }
