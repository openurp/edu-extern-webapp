...
[@b.form name="otherGradeForm" action="!save" theme="list"]
    ...
    [@b.select name="otherGrade.subject.id" items=otherExamSubjects?sort_by("name") empty="..." required="true" label="考试科目" value=(otherGrade.subject.id)! style="width:150px"/]
    [@b.field label="考试学期"]
...
<script language="javascript" >
    ...
    function searchStudent(){
        jQuery.post("otherGrade!searchStudent.action",{studentCode:$("#stdCode").val()},function(data){
            if (data == ""){
                $("#stdId").val("");
